...
function InspectionRow({ result, selected, onSelect, onModelTest, onToggle, onAction }: { result: InspectionResult; selected: boolean; onSelect: () => void; onModelTest: () => void; onToggle: () => void; onAction: () => void }) {
  ...
  return (
    <tr className={result.run_id ? "inspection-result-observed" : ""}>
      ...
      <td data-label={tx("ui.healthy")}><span className={`health-badge health-${result.health}`}><span />{healthLabel(result.health, locale)}</span></td>
      <td data-label={tx("ui.accounts")}><div className="inspection-account"><strong>{result.name || result.id}</strong><code>{result.id}</code></div></td>
      <td data-label={tx("ui.type")}><div className="inspection-type"><strong>{result.provider || tx("ui.unknown")}</strong><span>{result.plan_type || result.type || "-"}</span></div></td>
      <td data-label={tx("ui.quota_and_usage")}><InspectionQuotaUsage result={result} /></td>
      <td data-label={tx("ui.decision")}><div className="inspection-reason"><strong>{reasonLabel(result.reason_code, locale)}</strong><span>{result.status_code ? `HTTP ${result.status_code} · ` : ""}{confidenceLabel(result.confidence, locale)}{result.signal_source ? ` · ${signalSourceLabel(result.signal_source, locale)}` : ""}</span>{result.review_status ? <small className={`review-state review-${result.review_status}`}>{reviewStatusLabel(result.review_status, locale)}</small> : null}</div></td>
      <td data-label={tx("ui.model_probe")}><div className={`inspection-probe probe-${result.probe_status || "none"}`}><strong>{result.probe_reason_code ? reasonLabel(result.probe_reason_code, locale) : tx("ui.no_probe_result")}</strong><span>{result.probe_kind === "credential" ? tx("ui.credential_preflight") : result.probe_model || "-"}{result.probe_latency_ms ? ` · ${result.probe_latency_ms} ms` : ""}</span>{result.probe_tested_at ? <time title={tx("ui.last_model_probe_time", { time: formatDateTime(result.probe_tested_at) })}>{formatDateTime(result.probe_tested_at)}</time> : null}</div></td>
      <td data-label={tx("ui.streak")}><div className="inspection-streak"><span className="danger">{tx("ui.failures_count", { count: result.failure_streak })}</span><span className="success">{tx("ui.recovery_count", { count: result.healthy_streak })}</span></div></td>
      <td data-label={tx("ui.recommendation")}><span className={`recommendation recommendation-${result.recommendation}`}>{recommendationLabel(result.recommendation, locale)}</span></td>
      <td data-label={tx("ui.automation")}><div className="inspection-action-state"><strong>{result.circuit_open ? tx("ui.passive_temporary_circuit") : actionLabel(result.auto_action, locale)}</strong><span>{result.circuit_open && result.recover_after ? tx("ui.circuit_recovers_at_time", { time: formatDateTime(result.recover_after) }) : actionStatusLabel(result.auto_action_status, result.owned_disable, locale)}</span></div></td>
      <td data-label={tx("ui.checked")}><time>{formatDateTime(result.last_checked_at)}</time></td>
