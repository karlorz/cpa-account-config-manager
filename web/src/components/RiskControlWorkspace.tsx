...
type RiskTab = "content" | "audit" | "custom";
...
function AuditStatus({ status, source, formatNumber, tx }: { status?: RiskAuditModuleStatus; source?: RiskAuditModelSource; formatNumber: (value: number) => string; tx: (key: any, values?: any) => string }) { if (!status) return <div className="risk-audit-status muted">{tx("ui.risk_audit_status_unavailable")}</div>; const native = source === "account" || source === "ai_provider"; const availabilityLabel = native ? (status.api_key_available ? tx("ui.risk_audit_cpa_model_ready") : status.api_key_configured ? tx("ui.risk_audit_cpa_model_unavailable") : tx("ui.risk_audit_cpa_model_unset")) : (status.api_key_available ? tx("ui.risk_audit_api_key_ready") : status.api_key_configured ? tx("ui.risk_audit_api_key_unavailable") : tx("ui.risk_audit_api_key_unset")); return <div className="risk-audit-status" role="status"><div><span>{tx("ui.risk_audit_active")}</span><strong>{status.active ? tx("ui.enabled") : tx("ui.disabled")}</strong></div><div><span>{tx("ui.risk_audit_queue")}</span><strong>{formatNumber(status.queue_length)} / {formatNumber(status.queue_capacity)}</strong></div><div><span>{tx("ui.risk_audit_processed")}</span><strong>{formatNumber(status.processed)}</strong></div><div><span>{tx("ui.risk_audit_blocked")}</span><strong>{formatNumber(status.blocked)}</strong></div><div><span>{tx("ui.risk_audit_errors")}</span><strong>{formatNumber(status.errors)}</strong></div><div><span>{native ? tx("ui.risk_audit_cpa_model") : tx("ui.risk_audit_api_key")}</span><strong>{availabilityLabel}</strong></div></div>; }
...
function AuditForm({ value, status, accounts, providers, accountModels, accountModelsLoading, onChange, tx, formatNumber }: AuditFormProps) {
  ...
  return <div className="risk-audit-panel">
    <div className="risk-audit-intro"><div><h3>{tx("ui.risk_audit_title")}</h3><p>{tx("ui.risk_audit_description")}</p></div><AuditStatus status={status} source={source} formatNumber={formatNumber} tx={tx} /></div>
