...
const account: Account = {
  ...
  header_count: 2,
	concurrency: { supported: true, active: 10, limit: 0, request_limit: 0, request_window_seconds: 15, used_requests: 3, waiting: 0 },
	model_policy: {
		mode: "allow_only",
...
it("shows only the safe account detail model and opens single-account editing", async () => {
  ...
  expect(screen.getByText("Authorization")).toBeInTheDocument();
	...
	expect(screen.getByText("并发 10/∞ · 15s 请求 3/∞ · 队列 0")).toBeInTheDocument();
...
it("shows live active concurrency, request window, and queue", () => {
	render(<AccountDetailsDialog account={{ ...account, concurrency: { supported: true, active: 10, limit: 100, request_limit: 3, request_window_seconds: 15, used_requests: 2, waiting: 0 } }} onClose={() => undefined} onEdit={() => undefined} />);
	expect(screen.getByText("并发 10/100 · 15s 请求 2/3 · 队列 0")).toBeInTheDocument();
});
