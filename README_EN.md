...
- Overview gains a **usage by product** split: each product states the tokens, requests and reference-priced amount that can be attributed to it. OpenCode counts only this plugin's own OpenCode channels, Codex adds its credentials' own usage to its codex-api-key channels, and everything else is reported under "other providers". The split uses exactly the two stores the totals above add up, so its rows always sum back to those totals; traffic that cannot be attributed stays in the totals and appears in the "other providers" row instead of disappearing from the breakdown.
...
- Policy notifications also support **state triggers** that alert when accounts lose authorization, get disabled, or keep failing. A state rule matches the latest inspection evidence on `health`, `reason_code`, `status_code`, `disabled`, or `disable_reason`, and its scope decides whether any account, every account, or at least N accounts must match — for example "every account is disabled" or "error code = 429". A state rule and the availability thresholds are independent triggers, and the new `${state_rule}` and `${matched_accounts}` template variables describe a state-triggered delivery. This capability resolves [issue #5](https://github.com/Mxucc/cpa-account-config-manager/issues/5).
...
- Overview gains a **quota windows** panel: it reads each Codex credential's own 5-hour / 7-day usage snapshot and reports the share and reset time of the account using the most (the runtime percentages are this plugin's own budget policy, so they are not used here). When no credential reports a window, the panel is not rendered at all instead of showing 0%.
- Overview's Codex account count and Models and prices' per-row account count come from the Codex credentials the host lists (native OAuth accounts included) and their effective model catalogs, not from runtime provider identities: a deployment whose Codex credentials are all native accounts no longer reports zero accounts and an empty model list.
...
- The Codex automatic model allow-list is a visible experiment in **Other settings → Experimental features**, off by default. When it is on, it only triggers after an account's default probe model is rejected with the known ChatGPT-account incompatibility response and the fallback model works: the plugin then probes every model in that account's catalog (at most 32, three at a time, one 60s budget) and writes the allow-list only when every probe returned a verdict; otherwise it writes nothing and journals `insufficient_compatibility_evidence`. A written policy is marked as automatically detected (`auto_detected`, `detected_at`, `verified_models`), and the experimental panel shows how many accounts are currently auto-limited with the newest detections. An existing manual allow-list or deny-list is never overwritten.
- Both settings are stored in the plugin private data directory (0600) and exposed through management routes that require the Management Key, under `/v0/management/plugins/cpa-account-config-manager`: `GET /codex/overview`, `GET|PUT /codex/fingerprint`, `POST /codex/fingerprint/reset`, `GET|PUT /codex/models`, and `GET /experiments/auto-model-whitelist`. Responses never include a credential.

### OpenCode

A dedicated **OpenCode** workspace is added to the side menu directly after **AI Providers**:

- The workspace is organized as tabs for easier management, in this order: Overview (OpenCode cumulative usage and quota windows, billing summary, conversation-session status, and a counts strip), Go accounts, Zen accounts, Channels, and Models and prices (the price catalog plus the model test). The quick links and the refresh action stay available on every tab, and opening a model test from an account row switches to the Models and prices tab.
- Overview leads with OpenCode's own usage: total tokens (input / output / cached split), the request count, and a **reference-priced** amount at OpenCode's published prices. The money figure only sums runtime aggregates that can be proven to belong to this plugin's own OpenCode channels - Go and Zen credentials are attributed through their account or workspace id - so unattributable traffic is never counted, and is never shown as a zero. Below it the 5-hour / 7-day / 30-day quota windows report the tightest account per window with its reset countdown, together with the reference-priced usage the runtime attributes to the 5-hour / 7-day windows; when no OpenCode traffic has been recorded at all, the section says so instead of printing zeroes.
...
- A Cline Pass token expires on its own, and CPA only routes through the key stored on its channel row, so an expired or rotated token shows up as an authorization error on the AI-provider account that asks for a new sign-in while the account page still reports it bound. Every read of the Cline Pass account page or model page now repairs that automatically: a token inside the refresh margin is rotated first, then the account's current token is compared with the key on its own channel row - matched by its unique account label - and a row still holding the superseded key is rewritten. No manual "refresh sign-in", and no deleting and re-adding the AI-provider account. While the repair cannot run, the account is honestly reported unbound instead of looking usable, the repair keeps the existing 30-second throttle, and both outcomes are recorded in the operation history (`channel_bound` / `channel_bind_failed`). A duplicated label (several accounts sharing the default one) is never guessed at and is left untouched.
- A token can also be refused **before** its recorded expiry (the upstream answers `Unauthorized: ... re-authenticate your Cline account`, CPA cools the credential and then reports `auth_unavailable: no auth available` with status 503). That is repaired automatically too: the plugin recognises the rejection in the request-completion callback (status 401/403, or 503 whose message carries an authorization marker), attributes it to the stored account through the recorded auth index or auth id - never guessing when the request names no account - and then rotates that account's token and rewrites its own channel row. Until that finishes, the account page reports "Credential rejected; repairing automatically" and marks the account unroutable rather than claiming it is bound, and the rejection is never swallowed silently: the operation history records `cline_pass_credential_rejected`. The automatic repair coalesces a burst into one rotation plus one channel write with a 15-second backoff and throttles rotations to one per account per minute; writing a channel row needs the Management Key, which the plugin remembers from the last management request - in memory only, never logged or persisted. If the write keeps failing, or the credential genuinely needs a new sign-in, the page says so and signing in again once is the remaining step.
...
- **Model error** is its own log category: every model request that finally fails writes one entry with the **original upstream error text** (with `Bearer`/`sk-...`/`at-...`/`api_key=...` shapes stripped and a length cap, and no credential ever written), the upstream status code, the model, the retries it was allowed and the request id, filterable in the Audit Log. Once the retry budget is spent the plugin answers with **503** (`code=retry_exhausted`, `retry_limit`, `request_id`) instead of passing the upstream status through; a client cancel is recorded as cancelled. The host does not let a plugin rewrite an **already failed** response, so the 503 is implemented by intercepting the attempt that would follow the budget: the published per-credential budget is one higher than the setting, and the plugin uses exactly that attempt to answer 503, while the operator still gets every configured retry.
...
- One gateway can carry **several bound accounts at once**: every account publishes its own `openai-compatibility` channel row (labelled with the account name, or the account id for an account that was never renamed), and neither credentials nor model lists interfere. CPA selects a credential per row, so while one account is rate-limited the sibling row keeps serving the same models.
- **A quota-limited account is temporarily disabled**: when the Cline gateway answers `429` (`INFERENCE_CAP_ERROR`, for example "The limit resets in 2d 22h"), the plugin records the window the gateway itself named - 15 minutes, deliberately short, when none parses, and clamped at both ends - and temporarily sets `disabled` on **that account's own channel row**, so CPA stops routing to a credential with no allowance left and a sibling account of the same gateway serves the models. The row is enabled again once the window ends, or as soon as a model test or a routed request proves the account works; the row and its published model list are kept, so nothing has to be re-bound and usage attribution never breaks. The account page says so honestly ("Temporarily disabled (quota)") together with the moment routing resumes.
  - Two rules keep it from doing harm: **only a row this plugin disabled is ever enabled by this plugin** - a row the operator switched off by hand is never turned back on, not even when a quota record has run out - and **every 429 re-confirms that the row is out of routing**, even a rejection that repeats the window already recorded (an unchanged record, or a record that could not be written, still takes the row out), so "the test says unusable while the channel is still being scheduled" cannot happen. A failed store write is journaled, and the model test response carries what the call did (`quota.row`, `quota.row_error`).
  - Triggers: a `quota_limited` model-availability test result (the test used in the report), the quota message of a request completion, and the maintenance pass either of them schedules - the same pass that repairs rejected credentials, which also serializes the two writers of the channel list. A row this plugin disabled is never released by a "successful" completion: nothing can have gone through a row that is out of routing, so such a completion is a misattribution.
  - Both the window and the plugin-side disable flag are persisted, so a restart neither lifts a hold nor enables a row by mistake; a repeated rejection rewrites the store only when the window moved out noticeably.
- **A token rotation and the channel row move together**: an OAuth access token is rotated before it expires (reading the accounts page, testing a model and loading the model catalog all rotate). A rotation invalidates the token CPA is still routing through, so every rotation republishes that account's own channel row **in the same request** - a manual model test or model-list reload can no longer be what breaks every other call with an authorization error.
  - A rotation the gateway refuses (`invalid_grant`, 401/403 - the failures that need a new sign-in) is journaled (`cline_pass_token_refresh_failed`), marked on the account, and shown as a reason on the accounts page (`channel_binding_error`) instead of only "not bound". A transient failure (a timeout, a gateway 5xx) is not treated as "sign in again": the stored token keeps working until it expires and the maintenance pass retries.
  - **The expiry is whatever the gateway reports**: a rotation answer carrying `expiresAt` / `expiresIn` is recorded as given (bounded when it is nonsense) instead of assuming a fixed 55 minutes, which is what used to leave a stored credential looking valid long after the gateway had stopped accepting it. Without a reported expiry the documented lifetime is used (55 minutes minus the 5-minute refresh margin).

  - **The credential is renewed even when nobody is looking at the page**: the first request that carries a management key starts a background upkeep pass (every 3 minutes) that rotates the tokens inside the refresh margin, republishes the rows that no longer carry the live token and aligns the quota holds. That closes the path where nobody opened the Cline Pass page for a while, the token lapsed and every call through CPA failed with an authorization error. The pass only runs once this process has seen a management key - the key stays in memory and is never persisted - so after a CPA restart one plugin page load hands it a key again.
  - **A refusal has a second source**: CPA's usage callback carries the upstream status and body, so the plugin recognises a refused credential there as well (401/403, or 503 without a status and an authorization marker in the body), attributes it through the auth index or the credential digest, and starts the same repair even when the request-completion callback is not delivered. An ordinary 5xx or a rate limit is never mistaken for a credential that needs a new sign-in.
  - **A manual test starts the repair too**: when the plugin's model test or the AI-provider channel test is refused by the gateway with 401/403 (`authentication_failed`, `unauthorized: ... re-authenticate your Cline account`), the account is marked as rejected and its token is rotated and its own row rewritten - the test no longer just reports the 401 and leaves the row as it was. Attribution tries the account id and the stored token first, then the digest of the credential the last bind wrote into that row (a row may still hold the pre-rotation token).
- **Overview** is the default first screen: it sums the three documented windows (rolling 5 hours, calendar week, calendar month) into request counts, tokens and **reference-priced** amounts, and compares the month's **used allowance** against the $9.99 subscription. The plugin no longer derives a remaining balance, because a balance priced at reference rates is not the subscription allowance that is actually left; the used amount is the figure that can be measured. The amounts use Cline's published reference rates, so they are a "what this would have cost at standard API rates" comparison, never a bill. Cline's gateway publishes the DeepSeek subscription model as `cline-pass/deepseek-v4.1-flash`, while the documented reference table prices "DeepSeek V4 Flash" - DeepSeek retired `deepseek-v4-flash` and serves that model as DeepSeek-V4.1-Flash at the Flash price - so the plugin resolves both spellings to the one documented row instead of reporting the model as free. A model Cline publishes no reference rate for (for example `cline-pass/glm-5.3-flash`) is still shown as unpriced together with its unpriced request count, so a zero is never mistaken for free usage.
- Historical usage repair: usage recorded before a price table learned a model stayed unrated forever. Reading the CPA channel list, and opening the usage route, now re-price those rows whenever the rate can be applied exactly to a sum of requests, moving their counters from the unpriced side to the priced side. A tiered rate (long-context or priority) cannot be applied to an aggregate, so such rows stay unpriced instead of being estimated. The pass only adds to the totals: it never writes into a rolling window, and it survives a restart.
- The window events are written to the plugin's private store next to the account state, so a restart or a hot update no longer resets the three windows to zero. Only events still inside a window are kept, both the key count and the events per key are bounded, and an event that has aged out is never restored.
