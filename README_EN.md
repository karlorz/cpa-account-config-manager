...
- **A quota-limited account is temporarily disabled**: when the Cline gateway answers `429` (`INFERENCE_CAP_ERROR`, for example "The limit resets in 3d 13h"), the plugin records the window the gateway itself named - 15 minutes, deliberately short, when no window parses - and temporarily sets `disabled` on **that account's own channel row**, so CPA stops routing to a credential with no allowance left. The row is enabled again once the window ends, or as soon as a model test or a routed request proves the account works. The row and its published model list are kept, so nothing has to be re-bound while it is out, and usage attribution never breaks. The account page says so honestly ("Temporarily disabled (quota)") together with the moment routing resumes. The triggers are a `quota_limited` model-availability test result, the quota message in a request completion, and the maintenance pass either of them schedules; the record is persisted, so a restart does not lift the hold.
