...
- **Overview** is the default first screen: it sums the three documented windows (rolling 5 hours, calendar week, calendar month) into request counts, tokens and **reference-priced** amounts, and compares the month against the $9.99 subscription. The amounts use Cline's published reference rates, so they are a "what this would have cost at standard API rates" comparison, never a bill. Cline's gateway publishes the DeepSeek subscription model as `cline-pass/deepseek-v4.1-flash`, while the documented reference table prices "DeepSeek V4 Flash" - DeepSeek retired `deepseek-v4-flash` and serves that model as DeepSeek-V4.1-Flash at the Flash price - so the plugin resolves both spellings to the one documented row instead of reporting the model as free. A model Cline publishes no reference rate for (for example `cline-pass/glm-5.3-flash`) is still shown as unpriced together with its unpriced request count, so a zero is never mistaken for free usage.
