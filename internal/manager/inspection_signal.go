...
func accountQuotaRecoveredAfterDisable(account Account, record inspectionRecord, now time.Time) (string, bool) {
	...
	if snapshot := account.Usage.Quota; snapshot != nil {
		if !freshQuotaRecoveryObservedAt(snapshot.ObservedAt, record.DisabledAt, now) || quotaRecoveryBlockedByFailure(account, record, now) {
			return "", false
		}
