...
// renderOpenCodeStatusPage renders the standalone OpenCode Go status page. The
// page is read-only: it shows cached quota state and masks workspace identifiers,
// because the resource route behind it is not authenticated. Configuration and
// refresh happen in the authenticated plugin UI.
func renderOpenCodeStatusPage(snapshot OpenCodeQuotaSnapshot, message string) string {
	var statusHTML string
	if len(snapshot.Accounts) == 0 {
		statusHTML = `<div class="status-bar status-empty">No accounts bound yet. Bind a workspace from the CPA-A Manager settings page.</div>`
	} else {
		statusHTML = `<div class="status-bar">Accounts bound: ` + strconv.Itoa(len(snapshot.Accounts)) + `</div>`
	}
	...
	if len(snapshot.Accounts) == 0 {
		...
		for _, account := range snapshot.Accounts {
			...
			builder.WriteString(`<div><div class="account-title">` + html.EscapeString(maskOpenCodeWorkspaceID(account.WorkspaceID)) + `</div><div class="account-sub">Workspace</div></div>`)
	...
	messageHTML := ""
	if message != "" {
		messageHTML = `<div class="hint" style="margin-bottom:14px">` + html.EscapeString(message) + `</div>`
	}
	...
	return `<!doctype html><!-- opencode-go-quota --><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light dark"><title>OpenCode Go Quota</title><style>` + openCodeStatusCSS + `</style></head><body><div class="wrap"><div class="head"><h1>OpenCode Go</h1><div class="sub">Multi-account quota monitor · Last refresh ` + html.EscapeString(fetched) + `</div></div>` +
		statusHTML +
		messageHTML +
		content + `<div class="foot">Read-only status view. Credentials are stored only in the plugin private data directory and are never returned to the browser or written to logs; bind or refresh workspaces in the authenticated CPA-A Manager settings.</div></div></body></html>`
}

// maskOpenCodeWorkspaceID keeps a short suffix so an operator can still tell two
// workspaces apart without exposing the full identifier on an unauthenticated page.
func maskOpenCodeWorkspaceID(value string) string {
