...
- 「总览」是默认首屏：把三个官方窗口（滚动 5 小时、自然周、自然月）的请求数、Token 与**参考价**金额汇总展示，并与 9.99 美元/月的订阅价对比余额。金额按 Cline 官方公布的参考价折算，只是「按标准 API 价折算」的对照值，不是账单。Cline 网关把 DeepSeek 订阅模型发布为 `cline-pass/deepseek-v4.1-flash`，而官方参考价表按「DeepSeek V4 Flash」计价——DeepSeek 已下线 `deepseek-v4-flash` 并由 DeepSeek-V4.1-Flash 以 Flash 价格承接——插件把两种写法归一到同一行参考价，因此该模型的用量不会再显示成 0。官方未公布参考价的模型（例如 `cline-pass/glm-5.3-flash`）仍按「未定价」显示并给出未定价请求数，避免把 0 当成免费。
