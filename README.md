...
- **额度受限时临时停用**：当 Cline 网关回 `429`（`INFERENCE_CAP_ERROR`，如「The limit resets in 3d 13h」）时，插件记录网关自己给出的重置窗口（无法解析时按 15 分钟保守处理），并把**该账号自己的渠道行**临时置为 `disabled`，让 CPA 停止向这个已耗尽额度的凭据路由；窗口结束，或一次模型测试／真实请求证明该账号已恢复时，插件会自动重新启用该行。行本身与已发布模型列表都会保留，因此停用期间无需重新绑定，用量归因也不会断。停用期间账号页会如实显示「已临时停用（额度受限）」与恢复时间。触发点是模型可用性测试的 `quota_limited` 结果、请求完成回调里的额度报文，以及这两者触发的维护流程；记录会持久化，重启后不会被自动解除。
